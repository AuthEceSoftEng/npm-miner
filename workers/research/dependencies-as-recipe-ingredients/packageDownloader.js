/*
    file: packageDownloader.js
    objective: A crawler to just download packages from npm in a sequence and ask GitHub and npms.io about some stats
*/ 
...
const limit = 1000;
const { start_page } = JSON.parse(fs.readFileSync("start_page.json"));
...
MongoClient.connect(url, { useNewUrlParser: true }).then((client) => {
    ...
    .then(body => {
        ...
        return Promise.each(pages, (page) => {
                ...
                return Promise.mapSeries(body.rows, apackage => {
                    ...
                    if(
                        ...
                        return Promise.all(remote_tasks).then(function(res) {  
                            ...
                            return collection.updateOne({name: package.name}, {$set:{
                                name: package.name,
            ...
            fs.writeFileSync("start_page.json", JSON.stringify({ "start_page": at_page}));
            logger.error(err);
            process.exit(1);
        })
    });
