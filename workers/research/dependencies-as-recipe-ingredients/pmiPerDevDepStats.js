...
MongoClient.connect(url, { useNewUrlParser: true }).then((aclient) => {
    ...
    // Output the top devDependencies according to count
    const top = convertArrayToCSV(countingToArrayWithKey);
    fs.writeFileSync('output-topDevDeps.csv', top);

    // Order the devDependencies and get the names of them
    ...
    db = {}; // A database of PMI information for pairs of devDependencies
    ...
    // Write for each pair of devDependencies its pmi
    fs.writeFileSync('output-pmiPerDevPair.csv', file);

    // Calculate stats for recipes/pairs...
    let stats = [];
    _.map(packages, package => {
        const devDependencies = _.keys(package.latest_package_json.devDependencies);
        let combinations = [];
        // Since you only want pairs, there's no reason
        // to iterate over the last element directly
        for (let i = 0; i < devDependencies.length - 1; i++) {
        // This is where you'll capture that last value
            for (let j = i + 1; j < devDependencies.length; j++) {
                combinations.push(`${devDependencies[i]};${devDependencies[j]}`);
            }
        }
        let min = 100;
        let max = 0;
    ...
    fs.writeFileSync('output-pmiStatsPerDevPackage.csv', file);
}).finally(() => {
    client.close();
});